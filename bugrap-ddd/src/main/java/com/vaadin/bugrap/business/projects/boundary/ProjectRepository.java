...
public class ProjectRepository {
    ...
    public List<Report> getUnassignedReports(Project project) {
        return this.em.createNamedQuery(Report.unassignedReports)
                .setParameter("proj", project).getResultList();
    }
     ...
     */
    public Reporter getReporterByNameOrEmail(String username, String email) {
        return (Reporter) this.em
                .createNamedQuery(Reporter.reporterByNameOrEmail)
                .setParameter("email", email).setParameter("name", username)
                .getSingleResult();
    }
    ...
    public List<Comment> getComments(Report report) {
        return this.em.createNamedQuery(Comment.commentsForReport)
                .setParameter("rep", report).getResultList();
    }
    ...
    public List<Report> getLatestReports(int amount) {
        return this.em.createNamedQuery(Report.latest).setMaxResults(amount)
                .getResultList();
    }
    ...
    public long countReports(ProjectVersion version, ReportStatus status) {
        return (Long) this.em.createNamedQuery(Report.countWithStatus)
                .setParameter("pv", version).setParameter("status", status)
                .getSingleResult();
     ...
     * @return
     */
    public List<Report> searchReports(String searchTerm, Project project,
            ProjectVersion version, ReportStatus status, ReportType type,
            ReportResolution resolution, ReportPriority priority,
            boolean ignoreProject, boolean ignoreProjectVersion,
            boolean ignoreStatus, boolean ignoreType, boolean ignoreResolution,
